...
export class DoctorService {
  ...
  private get apiUrl(): string {
    if (typeof window !== 'undefined') {
      ...
      if (port === '4200') {
        return `${window.location.protocol}//${hostname}:5000/api/doctors`;
      }
    }
  ...
  getDoctorById(
    ...
    if (
      ...
      this.http
        ...
        .subscribe({
          next: (doc) =>
            this.doctorDetailCache.set(id, doc)
        });
